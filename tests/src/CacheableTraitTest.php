...
/**
 * Tests \Xylemical\Cache\CacheableTrait.
 */
class CacheableTraitTest extends TestCase {
  ...
  public function testSanity(): void {
    /** @var \Xylemical\Cache\CacheableInterface $cacheable */
    $cacheable = $this->getMockForTrait(CacheableTrait::class);

    $this->assertEquals([], $cacheable->getCacheDependencies());
    $this->assertEquals(CacheMaxAge::FOREVER, $cacheable->getCacheMaxAge());

    $a = $this->getMockCacheable([], 10);
    $b = $this->getMockCacheable([$a], 5);
    $cacheable->mergeCacheable($b);
